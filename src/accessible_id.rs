...
use zbus::zvariant::{
	Error as ZvariantError, ObjectPath, OwnedObjectPath, OwnedValue, Signature, Type, Value,
};
	...
	fn try_from(path: &'a ObjectPath<'a>) -> Result<Self, Self::Error> {
		match path.split('/').next_back() {
			...
			None => Err(Self::Error::Message(
				"No ID in attempted conversion from a String to an AccessibleId".to_string(),
			)),
		}
	}
}
...
mod tests {
	...
	fn deserialize_root_object_path() {
		...
		let id: AccessibleId =
			serde_plain::from_str(root_str).expect("Can not deserialize {root_str}");
	...
	fn serialize_1337_object_path() {
		...
		let one_one_three_one_str =
			serde_plain::to_string(&id).expect("Could not deserialize {id}");
